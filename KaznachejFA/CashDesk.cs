//------------------------------------------------------------------------------
// 20.04.2018 Copyright Vladimir Pakhomov ga3@yandex.ru
//
...
namespace KaznachejFA
{
    ...
    static class CashDesk
    {
        ...
        private static CancellationToken CashDeskCT;

        public static bool Busy = false;

        public delegate void CashDeskAbortedDelegate();
        public delegate void CashDeskTaskStatusDelegate(int LowLevelTaskID, int HighLevelTaskID, KKTTaskStatus NewStatus);
        ...
        public static event CashDeskBytesDelegate CMDBytesWritten;
        public static event CashDeskTaskStatusDelegate KKTTaskStatusChanged;
        ...
        /// </summary>
        public static async void StartCommunication(CancellationTokenSource CTS)
        {
            while (true)
            {
                try
                {
                    CashDeskCT = CTS.Token;
#pragma warning disable CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
                    Task.Run(IncomingKKTDataWatcher, CashDeskCT);
                    Task.Run(KKTTaskWatcher, CashDeskCT);
...
#pragma warning disable CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
                    //эта задача нужна пока только для тестов, строчку Abort() - НЕ УДАЛЯТЬ
                    Task.Run(() => {
                        Abort();
                        //GetStageState();
                        //RequestKKTState();
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
                    while (true)
                    {
                        Task<UInt32> loadAsyncTask = CashDeskSerialDataReaderObject.LoadAsync(64).AsTask(CashDeskCT);
                        UInt32 bytesRead = 0;
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        public class KKTTask
        {
            ...
            public KKTTaskStatus Status
            {
                ...
                set
                {
                    KKTTaskStatusChanged?.Invoke(LevelV30PacketID, HighLevelTaskID, value);
                    switch (value)
                    {
                        case KKTTaskStatus.New:
                            {
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        /// </summary>
        public static void PrintReceipt(List<string> ReceiptLinesToPrint, double Quantity, int PricePerItem_MDE, int Cash_MDE, string ProductName, int TaxSystemInUse)
        {
            ...
            byte[] productnamebytes = EncodeStringToKKTBytes(ProductName);
            byte[] TaxSystemBytes = BCDHelper.IntToBcd2(false, TaxSystemInUse.ToString().PadLeft(2, '0'));
